...
class ISICDataset(Dataset):
    ...
    def __getitem__(self, index):
        img_name = os.path.join(self.img_path, f"{self.targets.image.iloc[index][:-4]}.jpg")
...
def create_skin_dataset(batch_size):
    ...
    data_set_train = ISICDataset(train_img_path, transform=data_transforms)
    ...
    dataloader_train_no_aug = data.DataLoader(
        TensorDataset(xs[:n_train], ys[:n_train], es[:n_train]), batch_size=batch_size, shuffle=True, num_workers=2
    )
    dataloader_val = data.DataLoader(
        TensorDataset(xs[n_train:], ys[n_train:], es[n_train:]), batch_size=batch_size, shuffle=True, num_workers=2
    )
    dataloader_ood = data.DataLoader(
        TensorDataset(xs[n_train:], ys[n_train:], es[n_train:]), batch_size=batch_size, shuffle=True, num_workers=2
    )
    """
...
def create_embedded_dataloader(dataloader, model, batch_size=32):
    ...
    dataloader_embedding = data.DataLoader(
        TensorDataset(xs, ys, es), batch_size=batch_size, shuffle=True, num_workers=2
    )
